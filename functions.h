...
#ifndef MATH_LAB_FUNCTIONS_H
#define MATH_LAB_FUNCTIONS_H
...
bool assign(const char* lhs, const char* rhs, CVarDB &database);
bool unary_op(const char* variable, OP unary_op,CVarDB &database);
bool binary_op(const char* left, OP op, const char* right, CVarDB &database);
bool binary_assign(const char* assign_var, const char* left, OP op, const char* right, CVarDB &database);
bool IsACompoundOp(char* Buffer, int segmt[][2], char* operand_1, char* operand_2, OP& op);
bool compound_op(char* operand_1, OP op, char* operand_2, CVarDB &database);
...
/**
 * unary_op - This function handles the unary operation case
...
bool unary_op(const char* variable, OP unary_op, CVarDB &database)
{
    ...
    int row = curr_value.getnR();
    int col = curr_value.getnC();
    add_value = *new CMatrix(row, col);
    for (int i = 0; i < row; i++)
    {
        for (int j = 0; j < col; j++)
        {
            add_value.values[i][j] = 1;
        }
    }
    switch(unary_op){
        case PRE_INC: //++x
            **database.search(variable) = curr_value + add_value;
            ...
            std::cout << **database.search(variable);
            return true;
        case PRE_DEC: //--x
            **database.search(variable) = curr_value - add_value;
            std::cout << variable << " =" << std::endl;
 ...
 * @return True if the assignment was successful, false if not
 */
bool compound_op(char* operand_1, OP compound_op, char* operand_2, CVarDB &database)
{
...
bool binary_op(const char* left, OP op, const char* right,CVarDB &database)
{
    return binary_assign("ans", left, op, right,database);
}
...
bool binary_assign(const char* assign_var, const char* left, OP op, const char* right, CVarDB &database)
{
    ...
    switch(op){ //Perform the appropriate mathematical operation
        ...
        case DIV: //(/)
            ...
            break;
        default: //otherwise
            std::cout << "BAD OPERATOR" ; //Operation fails
            return false;
            break;
    }
...
void Interpreter(std::string& Input,CVarDB &database)
{
        ...
        switch (iSeg) // valid cases can be : 2, 3 or 5
        {
            case 2: // the command line is possible to be unary operation
            {
                ...
                {   // it is a valid unary operation, output the result
                    unary_op(operand_1, op,database); //Call unary_op to handle variable manipulation
                }
                break;
            }
            case 3: // the command line is possible to be binary operation or assignment
            {
                ...
                {   // it is a valid assignment, output the result
                    Tag = true;
                    assign(res, operand_1, database); //Call assign to handle variable manipulation
                }
                ...
                {   // it is a binary operation, output the result
                    Tag = true;
                    binary_op(operand_1, op, operand_2, database); //Call bianry_op to handle variable manipulation
                }
                else if(IsACompoundOp(Buffer,segmt, operand_1, operand_2, op))
                {
                    Tag = true;
                    compound_op(operand_1, op, operand_2, database);
                }
                break;
            }
            case 5: // the command line is possible to be binary operation and assignment
            {
                ...
                if(Tag)
                {   // it is a binary operation, output the result
                    binary_assign(res, operand_1, op, operand_2,database); //Call binary_assign to handle variable manipulation
                }
                break;
            }
        }
    ...
    {
        std::cout<<"Sorry, do not understand." <<std::endl;
    }
        ...
        else
        {
            ...
            else if(Buffer[st]=='[')
            {
                ...
                while((ed<len)&&(Buffer[ed]!=']'))
                {
                    ...
                    {
                        std::cout<<"Expect a ']' here."<<std::endl;
                        error_code = false;
                        break;
                    }
                }
